(async () => {
  ...
  (async (sub) => {
    ...
    for await (const m of sub) {
      if (
        m.respond(sc.encode("RPI is Listening:s " + new Date().toISOString()))
      ) {
  ...
  (async (sub) => {
    ...
    for await (const m of sub) {
      // console.log(m)
      const [, , eventSubject] = m.subject.split(".");
      console.info(`[RPI] #${sub.getProcessed()} handling ${eventSubject}`);
      console.log(eventSubject);
      switch (eventSubject) {
        case RPI.EVENTS.ON:
          // send the number of seconds since up
          m.respond(
            sc.encode(
              `RPI is turned ON since ${(Date.now() - started) / 1000} seconds`
            )
          );
          break;
        case RPI.EVENTS.OFF: {
